...
class ThreadControllerTest {
    ...
    void checkGetThreads_whenNoThreadExists_thenSuccess() throws Exception {
        Mockito.when(threadService.paginatedThreads(any(Pageable.class))).thenReturn(new PageImpl<>(List.of()));
    ...
    void checkPostThreads_whenValidThread_thenCreated() throws Exception {
        ...
        mockMvc.perform(post(ThreadController.PATH + "/")
    ...
    void checkGetById_whenValidUser_thenIsReturned() throws Exception {
        ...
        mockMvc.perform(get(ThreadController.PATH + "/1"))
                .andExpect(status().isOk())
    ...
    void checkGetById_whenNoUser_then404Returned() throws Exception {
        Mockito.when(threadService.findById(any(Integer.class))).thenThrow(EntityNotFoundException.class);
    ...
    void checkGetUserfeedThreads_whenThreadExists_thenSuccess() throws Exception {
        Mockito.when(threadService.paginatedThreadsByUser(any(User.class),any(Pageable.class))).thenReturn(mockThreadPage);
        Mockito.when(userService.findById(any(Integer.class))).thenReturn(mockUser);

        mockMvc.perform(get(ThreadController.PATH+"/userfeed")
    ...
    void checkGetUserfeedThreads_whenNoThreadExists_thenSuccess() throws Exception {
        Mockito.when(threadService.paginatedThreadsByUser(any(User.class),any(Pageable.class))).thenReturn(new PageImpl<>(List.of()));
        Mockito.when(userService.findById(any(Integer.class))).thenReturn(mockUser);

        mockMvc.perform(get(ThreadController.PATH+"/userfeed").with(jwt().jwt(jwt -> jwt.claim("userId", mockUser.getId()))))
                .andExpect(status().isOk());
    }
}
