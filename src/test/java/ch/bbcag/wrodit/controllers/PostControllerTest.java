...
class PostControllerTest {
    ...
    void checkGetById_whenValidPost_thenIsReturned() throws Exception {
        ...
        mockMvc.perform(get(PostController.PATH + "/1"))
    ...
    void checkGetById_whenNoPost_then404Returned() throws Exception {
        Mockito.when(postService.getPostById(any(Integer.class))).thenThrow(EntityNotFoundException.class);
    ...
    void checkGetPosts_whenPostExists_thenSuccess() throws Exception {
        Mockito.when(postService.getPaginatedPosts(any(Integer.class), any(Integer.class), any(Pageable.class))).thenReturn(mockPostPage);

        mockMvc.perform(get(PostController.PATH+"/"))
                .andExpect(status().isOk());
    }
    ...
    void checkGetPosts_whenNoPostExists_thenSuccess() throws Exception {
        Mockito.when(postService.getPaginatedPosts(any(Integer.class), any(Integer.class),any(Pageable.class))).thenReturn(new PageImpl<>(List.of()));
