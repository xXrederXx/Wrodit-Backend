...
class PostServiceTest {
    ...
    {
        when(mockPostRepo.findAll(any(Specification.class), any(Pageable.class))).thenReturn(mockPostPage);
    ...
    void checkSave_whenValidPost_thenSuccess()
    {
        ...
        Assertions.assertTrue(OffsetDateTime.now().toEpochSecond() - result.getCreatedAt().toEpochSecond() < TestingUtil.MAX_TIME_CHECK_DIFF);
    ...
    void checkUpdate_whenInvalidChanges_thenThrows()
    {
        ...
        assertThrows(FailedValidationException.class, ()->postService.update(change, post.getId(), user.getId()));
    }
    ...
    void checkUpdate_whenNoAuth_thenThrows()
    {
        ...
        assertThrows(AuthorizationDeniedException.class, ()->postService.update(change, post.getId(), user.getId() + 1));
    }
}
