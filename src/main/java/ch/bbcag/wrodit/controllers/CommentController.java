...
public class CommentController {
    ...
    public ResponseEntity<?> getPagableComments(
        ...
        return ResponseEntity.ok(CommentMapper.toDto(commentService.getPaginatedComments(page, post, parent)));
    }
    ...
    public ResponseEntity<?> postComment(@RequestBody CommentCreateDTO commentCreateDTO,
        ...
        CommentResponseDTO responseDTO = CommentMapper.toDto(commentService.save(CommentMapper.fromDto(commentCreateDTO), authId));
    ...
    public ResponseEntity<?> patchComment(
            ...
            @RequestHeader(name = SecurityConstants.AUTH_HEADER_PASSWORD) String authPasswd) {
        userService.throwIfUnauthorized(authId, authPasswd);
        return ResponseEntity.ok(CommentMapper.toDto(commentService.update(CommentMapper.fromDto(dto), id)));
    }
    ...
    @ApiResponses(
            value = {
                    ...
                    @ApiResponse(responseCode = "404", description = "Comment was not found", content = @Content)
            })
